...
# mlp([10,20, 20, 10], nn.ReLU(), nn.Softmax(dim=1))
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        #print("Actor", obs.__class__)
        logits = self.logits_net(obs) # torch.from_numpy(obs)
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            # Compute policy distribution and log-probability of chosen action
    ...
    def get_action(self, obs):
        ...
        #obs = torch.from_numpy(obs).float() # np.ndarray vs tensor
        try: 
            pi = self.actor._distribution(obs) # .unsqueeze(0)
        except TypeError:
            pi = self.actor._distribution(torch.from_numpy(obs).float()) # .unsqueeze(0)
        
        try:
            probs = pi.probs.squeeze(0).numpy()
        except RuntimeError:
            probs = pi.probs.detach().numpy()
...
def train(env, seed=0):
    ...
    # ADAM optimizer using the parameters of the actor and then critic networks
    ...
    for epoch in range(epochs):
        ...
        # update the actor and / or critic function.

        data = buf.get()
        tdres = data['tdres'] # td residual/delta/phi
        obs = data['obs'] # state observation
        ret_buf = data['ret'] # discounted rewards to go, advantage
        act = data['act'] #action

        actor_optimizer.zero_grad()

        _, logp = agent.actor(obs, act)
        loss = -(tdres * logp).mean()
        loss.backward()
        ...
        for _ in range(100):
            ...
            value_loss = nn.MSELoss()(v, ret_buf)
            value_loss.backward()
...
def main():
    ...
    rec = VideoRecorder(env, "policy.mp4") # needed for the gif
    episode_length = 300
    n_eval = 100
    returns = []
